...
// .wrangler/tmp/bundle-Kjfc2m/checked-fetch.js
...
app.use("*", async (c, next) => {
  ...
  if (!userId) {
    ...
    setCookie(c, "user_id", userId, {
      ...
      secure: true,
      // Required for cross-origin cookies in production
      sameSite: "None",
...
app.get("/chat", async (c) => {
  ...
  const userId = c.get("userId");
  const memory = userMemory[userId];
  const contextMessage = memory.length ? `Here is what I know so far:
...
// .wrangler/tmp/bundle-Kjfc2m/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Kjfc2m/middleware-loader.entry.ts
